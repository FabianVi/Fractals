...
void Export::exportImage(Algorithm::Algo o,Vector2D<int> resolution, Vector2x2<long double> view, int depth, wxString filename) {
    auto* im_data = (unsigned char*) malloc(resolution.x * resolution.y * 3 );
    Algorithm::Fractal(o,im_data, resolution, depth, view);
 ...
 */
void Export::exportImages(Algorithm::Algo o,Vector2D<int> resolution, Vector2x2<long double> viewStart, Vector2x2<long double> viewEnd, int depth,int nrOfFrames, wxString path){
    ...
    for (int i = 0; i < nrOfFrames; ++i) {
        Export::exportImage(o,resolution,view,depth,(wxString::Format("%s/%d.bmp",path,i)));
